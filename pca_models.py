...
pca.fit(allBoxScores[1])
loadings = pd.DataFrame(pca.components_.T, columns=[f'PC{i+1}' for i in range(pca.n_components_)], index=allBoxScores[1].columns)
feature_importance = loadings.abs().sum(axis=1)
kept_features = feature_importance.nlargest(7).index.tolist()
