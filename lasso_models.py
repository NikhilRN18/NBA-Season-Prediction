...
lasso_best.fit(allBoxScores[1],allResults[1])
removed_features = np.array(allBoxScores[1].columns)[lasso_best.coef_ == 0]
kept_features = np.array(allBoxScores[1].columns)[lasso_best.coef_ != 0]
print("Features kept:",kept_features)
for i in allBoxScores:
    i.drop(removed_features,axis=1,inplace=True)
